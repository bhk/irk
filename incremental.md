...
During an update (after one or more inputs cells have changed), cells whose
...
Evaluation order in Rio is strict by default, but the `&` prefix operator
creates a lazy expression, whose contents will be evaluated at most once,
when and if the value is used.

The `memo` function constructs a cell to contain the lazy expression.
Evaluation of the cell is likewise on-demand.
...
This browser-hosted program retrieves a text file and displays it:

    showFile = (url) =>
       txt = try(IO.getURL(url))
       match txt:
          Error(e) => E.div(e.toString())
          Value(v) => E.plaintext(v)

    DOM.insert(showFile(window.location + "/example.txt"))

This program is a Rio module that evaluates to its last expression, which
constructs an action object that installs a node into the DOM tree.  As this
node changes over time, the "insertion" is replaced.

I/O operations like `getURL(url)` always evaluate to a cell.  On intial
evaluation, its result will be in an error state, and the error object will
indicate that it is in progress.  In time, this will transition to different
in-progress error states, and finally (on success) a non-error state.

The above code handles reporting errors, conveying progress (connecting,
etc..) prior to completion.  The `showFile` function is robust and
composable; no explicit registration or deregistration is necessary.
 ...
 * Bloated Code: Code must be written for objects to register for various
   type of changes, and to deregister, and on the notifying side to accept
 ...
 * "Red Tape": Objects must register for notifications when they use
